...
PRECISION = 64                    # fractional bits
SUB_PRECISION = 48                    # fractional bits
DECIMAL = 8

DECIMAL_ONE = uint256(100000000)
ETHER_ONE = uint256(100000000)
...
def exp(_x):
    ...
    elif (_x >> (PRECISION - SUB_PRECISION)) <= maxExpArray[SUB_PRECISION]:
        _x = _x >> (PRECISION - SUB_PRECISION)
        return int(fixedExp(_x, SUB_PRECISION)) << (PRECISION - SUB_PRECISION)
    else:
        assert(_x >> (PRECISION - MIN_PRECISION) <= maxExpArray[MIN_PRECISION])
        _x = _x >> (PRECISION - MIN_PRECISION)
        return int(fixedExp(_x, MIN_PRECISION)) << (PRECISION - MIN_PRECISION)
